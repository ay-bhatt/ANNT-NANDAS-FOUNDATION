        ...
        <div className="container-premium grid gap-10 lg:grid-cols-[1fr_1fr] lg:items-start">
          <div>
            ...
            <div className="space-y-4 text-base leading-8 text-slate-600">
              <p>
                <T>Nestled in the Himalayan village of Mundoli, Chamoli, Uttarakhand, ANNT NANDAS FOUNDATION is more than a non-profit organisation — it is a movement dedicated to transforming lives through opportunity, compassion, and purpose.</T>
              </p>
              <p>
                <T>Founded on 1st May 2023 as an unregistered community effort under the name Mundoli Riders Club, the work began with an extraordinary dream and the simplest of resources: two bicycles and twelve children. On 27 May 2026 the organisation was registered as अनंत नन्दा फाउण्डेशन (ANNT NANDAS FOUNDATION), a Section 8 non-profit.</T>
              </p>
              <p>
                <T>Every child possesses a gift. Some discover it early. Many never do. We believe talent is not determined by wealth, geography, or social status. It exists in every village, every home, and every child waiting for someone to believe in them.</T>
              </p>
              <p>
                <T>Whether that journey leads to becoming a national athlete, a skilled professional, an entrepreneur, a government officer, or a responsible citizen, we stand beside every child throughout the journey. We are not simply creating athletes. We are building confident leaders and transforming generations.</T>
              </p>
            </div>
            <div className="mt-8 grid gap-4 sm:grid-cols-2">
              <div className="surface-card p-5">
                <p className="text-sm font-semibold uppercase tracking-[0.2em] text-emerald-700"><T>Founded</T></p>
                <p className="mt-2 text-2xl font-bold text-slate-950">{siteConfig.founded}</p>
              </div>
              <div className="surface-card p-5">
                <p className="text-sm font-semibold uppercase tracking-[0.2em] text-emerald-700"><T>Registered</T></p>
                <p className="mt-2 text-2xl font-bold text-slate-950">{siteConfig.registered}</p>
              </div>
            </div>
          </div>
          <div className="grid gap-4 sm:grid-cols-2">
            {[foundersGallery[2], foundersGallery[3], foundersGallery[1]].filter(Boolean).map((image, index) => (
              <div key={index} className={`relative min-h-[220px] overflow-hidden rounded-[28px] ${index === 0 ? "sm:col-span-2 aspect-[4/5] sm:aspect-[16/10]" : "aspect-[4/5]"}`}>
                <Image src={image} alt={`Foundation story ${index + 1}`} fill sizes="(max-width: 767px) 50vw, 25vw" className="object-cover object-top" />
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="container-premium grid gap-8 lg:grid-cols-3">
          {[
            ...
            <article key={block.title} className="surface-card p-6 sm:p-7">
              <p className="text-[11px] font-semibold uppercase tracking-[0.2em] text-emerald-700"><T>{block.eyebrow}</T></p>
              <h2 className="mt-3 text-xl font-bold tracking-[-0.03em] text-slate-950"><T>{block.title}</T></h2>
              <p className="mt-4 text-sm leading-7 text-slate-600"><T>{block.body}</T></p>
            </article>
          ))}
        </div>
      </section>
          ...
          <div className="mx-auto max-w-4xl space-y-4">
            {journeyMilestones.map((item) => (
              <div key={item.year} className="surface-card flex gap-4 p-5">
                ...
                <div>
                  <h3 className="text-xl font-semibold text-slate-950"><T>{item.title}</T></h3>
        ...
        <div className="container-premium grid gap-10 lg:grid-cols-[1.05fr_0.95fr] lg:items-center">
          <div className="surface-dark relative overflow-hidden p-8 sm:p-10">
            <div className="absolute inset-0">
              <Image src={founderInfo.image} alt={`${founderInfo.name}, founder of ${ORG_NAME_EN}`} fill sizes="100vw" className="object-cover opacity-20" />
            ...
            <div className="relative z-10">
              ...
              <p className="mt-2 text-emerald-200"><T>{founderInfo.title}</T></p>
              <p className="mt-6 text-base leading-8 text-slate-200"><T>{founderInfo.fullBio}</T></p>
              <blockquote className="mt-6 border-l-2 border-emerald-300 pl-4 text-lg italic leading-8 text-white/90">
                “<T>{founderInfo.quote}</T>”
              </blockquote>
            </div>
          </div>
          <div>
            ...
            <div className="grid gap-3">
              {founderInfo.achievements.map((achievement) => (
                <div key={achievement} className="surface-card flex items-start gap-3 p-4">
                  ...
                  <p className="text-sm leading-7 text-slate-600"><T>{achievement}</T></p>
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
            {coreValues.map((value) => (
              <div key={value.title} className="surface-card p-6 text-center">
                ...
                <h3 className="mt-5 text-lg font-semibold text-slate-950"><T>{value.title}</T></h3>
                <p className="mt-3 text-sm leading-7 text-slate-600"><T>{value.desc}</T></p>
              </div>
            ))}
          </div>
        </div>
